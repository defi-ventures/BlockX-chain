...
Note, strict routability for addresses is turned off in the config file.`,
		...
		Example: "toknd testnet --v 4 --keyring-backend test --output-dir ./output --starting-ip-address 192.168.10.2",
	...
	cmd.Flags().String(flagNodeDaemonHome, "toknd", "Home directory of the node's daemon configuration")
	cmd.Flags().String(flagNodeCLIHome, "tokncli", "Home directory of the node's cli configuration")
	...
	cmd.Flags().String(flagCoinDenom, ethermint.AttoTokn, "Coin denomination used for staking, governance, mint, crisis and evm parameters")
	cmd.Flags().String(server.FlagMinGasPrices, fmt.Sprintf("0.000006%s", ethermint.AttoTokn), "Minimum gas prices to accept for transactions; All fees in a tx must meet this minimum (e.g. 0.01atokn,0.001stake)")
